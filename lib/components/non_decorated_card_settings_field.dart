...
class NonDecoratedCardSettingsField extends StatelessWidget implements ICommonFieldProperties {
      ...
      return Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[_buildLabelBlock(context), _buildInlineContent(context), _buildRightDecoration()],
            ),
  ...
  Widget _buildInlineContent(BuildContext context) {
    return contentOnNewLine ? Container() : Expanded(child: content);
  }
  ...
  Widget _buildLabelBlock(BuildContext context) {
    var padding = EdgeInsets.only(right: CardSettings.of(context)?.labelPadding ?? 6.0);
  ...
  Widget _buildLabelSpacer(BuildContext context) {
    return ((labelAlign ?? CardSettings.of(context)?.labelAlign ?? TextAlign.left) == TextAlign.right)
  ...
  TextStyle _buildLabelStyle(BuildContext context) {
    ...
    labelStyle = labelStyle.merge(Theme.of(context).inputDecorationTheme.labelStyle);
    if (!enabled) labelStyle = labelStyle.copyWith(color: Theme.of(context).disabledColor);
